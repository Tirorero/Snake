...
function map() {
    ...
    for (let i = 1; i <= document.getElementById("size").value; i++) {
        var new_div = document.createElement("div");
        new_div.classList.add("board-cell");

        // przypisywanie id od razu przy tworzeniu elementu
        new_div.setAttribute("id", "div" + i);

        document.getElementById("div0").append(new_div);
    }

    // te warunki wystarczyło sprawdzić raz, nie przy generowaniu każdej komórki
...
function up() {
    if (
        ...
        varup = setInterval(function () {
            let divs = document.getElementsByClassName("board-cell");
            ...
            for (let f = 0; f < fragments.length; f++) {
                document.getElementById("div" + fragments[f]).style.background =
                    "lightgreen";
            }
            ...
            while (l == j || fragments.includes(l) == true) {
                l = Math.floor(
                    Math.random() * document.getElementById("size").value + 1
                );
            }
    ...
    if (j != fragments[1] - 1) {
        ...
        varright = setInterval(function () {
            ...
            if (
                ...
                barrier_er.includes(j) == true &&
                document.getElementById("size").value == 1600
            ) {
                clearInterval(varright);
                alert("Porażka");
                return;
